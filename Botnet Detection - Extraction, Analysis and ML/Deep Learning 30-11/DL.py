...
def nn_model():
    ...
    model.add(Dense(100, input_dim = X_train.shape[1], kernel_initializer = 'he_normal'))
    model.add(PReLU())
...
def nn_model_dropout():
    ...
    model.add(Dense(50, input_dim = X_train.shape[1], kernel_initializer = 'he_normal'))
    model.add(Activation('sigmoid'))
    model.add(BatchNormalization())
...
earlyStopping = EarlyStopping(monitor='val_accuracy', patience=10, verbose=2, mode='min')


# In[ ]:

#model.fit(X_train,Y_train,epochs=200,batch_size=128,callbacks=[csv_logger,checkpointer],validation_data=(X_test,Y_test),verbose=2)
